...
exports.handler = async (event) => {
  ...
  try {
    ...
    switch (pathData) {
      ...
      // GET /admin/jobs/{job_id}/logs - Get CloudWatch logs for a Glue job
      case "GET /admin/jobs/{job_id}/logs":
        const jobId = event.pathParameters?.job_id;
        if (!jobId) {
          response.statusCode = 400;
          response.body = JSON.stringify({ error: "Job ID is required" });
          break;
        }

        // Get job to find glue_job_run_id
        const jobData = await sqlConnection`
          SELECT glue_job_run_id, textbook_id, status
          FROM jobs
          WHERE id = ${jobId}
        `;
        ...
        if (jobData.length === 0) {
          response.statusCode = 404;
          response.body = JSON.stringify({ error: "Job not found" });
          break;
        }

        const glueJobRunId = jobData[0].glue_job_run_id;
        if (!glueJobRunId) {
          response.statusCode = 404;
          response.body = JSON.stringify({
            error: "Job has not started yet - no Glue run ID available",
          });
          break;
        }
        ...
        try {
          ...
          const logStreamName = glueJobRunId;

          const logsParams = {
            logGroupName,
            logStreamName,
            startFromHead: true,
            limit: 1000, // Get last 1000 log events
          };

          const command = new GetLogEventsCommand(logsParams);
          const logsResponse = await cloudwatchLogsClient.send(command);

          response.statusCode = 200;
          response.body = JSON.stringify({
            job_id: jobId,
            glue_job_run_id: glueJobRunId,
            log_group: logGroupName,
            log_stream: logStreamName,
            events: logsResponse.events || [],
            nextForwardToken: logsResponse.nextForwardToken,
            nextBackwardToken: logsResponse.nextBackwardToken,
          });
        } catch (error) {
          ...
          if (error.name === "ResourceNotFoundException") {
            ...
            response.body = JSON.stringify({
              error:
                "Log stream not found - job may not have started logging yet",
              glue_job_run_id: glueJobRunId,
            });
          } else {
            response.statusCode = 500;
      ...
      // GET /admin/textbooks/{textbook_id}/logs - Get CloudWatch logs for a textbook's latest Glue job
      case "GET /admin/textbooks/{textbook_id}/logs":
        const logsTextbookId = event.pathParameters?.textbook_id;
        if (!logsTextbookId) {
          response.statusCode = 400;
          response.body = JSON.stringify({ error: "Textbook ID is required" });
          break;
        ...
        // Get the most recent job for this textbook
        const textbookJobData = await sqlConnection`
          SELECT id, glue_job_run_id, status
          FROM jobs
          WHERE textbook_id = ${logsTextbookId}
          ORDER BY created_at DESC
          LIMIT 1
        `;

        if (textbookJobData.length === 0) {
          response.statusCode = 404;
          response.body = JSON.stringify({
            error: "No job found for this textbook",
          });
          break;
        }

        const textbookGlueJobRunId = textbookJobData[0].glue_job_run_id;
        if (!textbookGlueJobRunId) {
          response.statusCode = 404;
          response.body = JSON.stringify({
            error: "Job has not started yet - no Glue run ID available",
          });
        ...
        try {
          // Fetch logs from CloudWatch
          const textbookLogGroupName = "/aws-glue/jobs/output";
          const textbookLogStreamName = textbookGlueJobRunId;

          const textbookLogsParams = {
            logGroupName: textbookLogGroupName,
            logStreamName: textbookLogStreamName,
            startFromHead: true,
            limit: 1000,
          };

          const textbookLogsCommand = new GetLogEventsCommand(
            textbookLogsParams
          );
          const textbookLogsResponse = await cloudwatchLogsClient.send(
            textbookLogsCommand
          );

          response.statusCode = 200;
          response.body = JSON.stringify({
            job_id: textbookJobData[0].id,
            textbook_id: logsTextbookId,
            glue_job_run_id: textbookGlueJobRunId,
            log_group: textbookLogGroupName,
            log_stream: textbookLogStreamName,
            events: textbookLogsResponse.events || [],
            nextForwardToken: textbookLogsResponse.nextForwardToken,
            nextBackwardToken: textbookLogsResponse.nextBackwardToken,
          });
        } catch (error) {
          console.error("Error fetching CloudWatch logs for textbook:", error);
          if (error.name === "ResourceNotFoundException") {
            response.statusCode = 404;
            response.body = JSON.stringify({
              error:
